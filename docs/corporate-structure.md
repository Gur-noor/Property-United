...
graph TD
    ...
    G --> E
    G --> F
    B -.->|Phase 3+| H[$TOKEN Foundation<br/>Offshore Issuer]
```
...
| $TOKEN Foundation (Phase 3+) | Offshore issuer of the platform utility token; licenses IP from PlatformCo | Established only at token launch |
...
## $TOKEN Foundation — Phase 3 and Beyond
...
The $TOKEN Foundation is described here for completeness of the entity architecture. Its formation and the associated token launch are contingent on a future decision point in the roadmap.
